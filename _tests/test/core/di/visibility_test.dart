  ...
  group('Visibility', () {
    group('local', () {
      test('component should not be injectable by child component', () async {
        final testBed =
            NgTestBed<ShouldFailToInjectParentComponent>(ng.createShouldFailToInjectParentComponentFactory());
        expect(testBed.create(), throwsNoProviderError);
      });

      test('directive should be accessible via a query', () async {
        final testBed = NgTestBed<ShouldQueryDirective>(ng.createShouldQueryDirectiveFactory());
      ...
      test('directive should be injectable on same element', () async {
        final testBed = NgTestBed<ShouldInjectFromElement>(ng.createShouldInjectFromElementFactory());
        final testFixture = await testBed.create();
        expect(testFixture.assertOnlyInstance.child!.directive, isNotNull);
      });

      test('directive should be injectable in same view', () async {
        final testBed = NgTestBed<ShouldInjectFromView>(ng.createShouldInjectFromViewFactory());
        final testFixture = await testBed.create();
        expect(testFixture.assertOnlyInstance.child!.directive, isNotNull);
      });
      ...
      test('directive should not be injectable in child view', () async {
        final testBed =
            NgTestBed<ShouldFailToInjectFromParentView>(ng.createShouldFailToInjectFromParentViewFactory());
        expect(testBed.create(), throwsNoProviderError);
      });

      test('directive should inject host component', () async {
        final testBed = NgTestBed<ShouldInjectHost>(ng.createShouldInjectHostFactory());
        final testFixture = await testBed.create();
      ...
      test('service on Visibility.none component is injectable', () async {
        final testBed = NgTestBed<MyComponentWithServiceTest>(ng.createMyComponentWithServiceTestFactory());
        var testFixture = await testBed.create();
        expect(testFixture.rootElement, isNotNull);
      });

      test('component may provide itself via another token', () async {
        final testBed = NgTestBed<ShouldInjectAliasedLocal>(ng.createShouldInjectAliasedLocalFactory());
        final testFixture = await testBed.create();
        expect(testFixture.text, testFixture.assertOnlyInstance.text);
      });

      test('directive may provide itself for a multi-token', () async {
        final testBed = NgTestBed<ShouldInjectMultiToken>(ng.createShouldInjectMultiTokenFactory());
        final testFixture = await testBed.create();
      ...
      test('should support $FactoryProvider', () async {
        final testBed =
            NgTestBed<ShouldSupportFactoryProvider>(ng.createShouldSupportFactoryProviderFactory());
        final testFixture = await testBed.create();
        expect(testFixture.assertOnlyInstance.child!.interface, isNotNull);
      });

      test('should support $ClassProvider', () async {
        final testBed = NgTestBed<ShouldSupportClassProvider>(ng.createShouldSupportClassProviderFactory());
        final testFixture = await testBed.create();
        expect(testFixture.assertOnlyInstance.child!.interface, isNotNull);
      });
    });
    ...
    group('all', () {
      test('component should be injectable by child component', () async {
        final testBed =
            NgTestBed<ShouldInjectParentComponent>(ng.createShouldInjectParentComponentFactory());
        final testFixture = await testBed.create();
